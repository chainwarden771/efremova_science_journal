...
import { POST_SOURCE_URL } from '../../api/app';
import { forwardRef } from 'react';

const Post = forwardRef(
  ({ headline, description, imageURL, rating, postID, publisherFullname, publishDate }, key) => {
    const navigate = useNavigate();

    return (
      <div key={key} className="post-item element-decoration--panel">
        ...
        <section>
          <div className="post-hat">
            <h1 className="h-header h1-header post-item__post-header">{headline}</h1>
            <img alt="post image" src={`${POST_SOURCE_URL}/${imageURL}`}></img>
          </div>
        ...
        <div className="post-item__footer">
          ...
          <Button className="post-item__read-full" onClick={() => navigate(`/read/${postID}`)}>
            Читать полностью
          </Button>
        </div>
      </div>
